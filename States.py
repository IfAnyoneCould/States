...
class Object(sprite.Sprite):
    def __init__(self,name,data):
        ...
        allSprites.add(self)
class State():
    def __init__(self,name,capitol,capinfo,bird,bGenus,flower,fInfo,des):
        self.data = [name,capitol,capinfo,bird,bGenus,flower,fInfo,des]
    ...
    def Load(self):
        ...
        self.flag.image = transform.scale(image.load("Sprites/flags/"+self.data[0][0]+".png"),(225,150)).convert()
        self.flag.rect = self.flag.image.get_rect(topleft=(25,75))
        self.capitol = text(self.data[1],350,25,30)
        self.capInfo = text(self.data[2],350,75,20)
        self.bird = text(self.data[3],25,375,30)
        self.birdGenus = text(self.data[4],25,400,20)
        ...
        self.flower = text(self.data[5],400,375,30)
        ...
        self.flowerInfo = text(self.data[6],550,450,12)
        self.des = text(self.data[7],25,250,15)
        self.sprites.add(self.flag,self.birdImg,self.flowerImg)
        self.text.add(self.name,self.capitol,self.capInfo,self.bird,self.birdGenus,self.flower,self.flowerInfo,self.des)
        return self
    def Render(self,window):
...
#maryland
marylandData = State(["Maryland"],["Capitol: Annapolis","Population: 38,394"],["Established in 1694","Rank in state: 7th"],["Baltimore oriole"],["Icterus galbula"],["Black-Eyed Susan","Coneflowers"],["Medicinal uses: The stem is an effective treatment for","those suffering from high blood pressure,","and the entire plant treats ulcers and bodily swelling."],["Maryland's flag bears the arms of the Calvert and Crossland families.","Calvert was the family name of the Lords Baltimore who founded Maryland,","and their colors of gold and black appear in the first and fourth quarters of the flag.","Crossland was the family of the mother of George Calvert, first Lord Baltimore."])
...
#michigan
michiganData = State(["Michigan"],["Capitol: Lansing","Population: 112,644"],["Established in 1847","Rank in state: 5th"],["American robin"],["Turdus migratorius"],["Dwarf Lake Iris","Irises"],["Medicinal uses: None, since it is an endangered species of flowers"],["Adopted in 1911","The state coat of arms depicts a blue shield, upon which the sun rises over a lake and peninsula,","and a man with a raised hand representing peace and holding a long gun representing the fight for state and nation as a frontier state."])
michigan = Object("Michigan",michiganData)
#minnesota
minnesotaData = State(["Minnesota"],["Capitol: Saint Paul","Population: 311,527"],["Established in 1849","Rank in state: 2nd"],["Common loon"],["Gavia immer"],["Pink & White Lady Slipper","Cypripedium"],["Medicinal uses: The root of lady's slipper was used","as a remedy for nervousness,","tooth pain, and muscle spasms."],["Adopted in 1983","The star represents 'L'etoile du Nord' and Minnesota's natural wealth,","the blue background represents Minnesota's lakes and rivers,","the white represents winter, and the green represents farmland and forests.","The waves represent the name Minnesota, a Dakota word which means 'sky-tinted waters'."])
...
#oregon
oregonData = State(["Oregon"],["Capitol: Salem","Population: 175,535"],["Established in 1859","Rank in state: 2nd"],["Western meadowlark"],["Sturnella neglecta"],["Oregon Grape","Mahonia"],["Medicinal uses: Oregon grape is used for scaly, itchy skin (psoriasis),","eczema (atopic dermatitis), stomach problems, and other conditions"],["Adopted 1925","The front side of the flag has the escutcheon","– or shield – that is found on the state seal.","There are 33 stars surrounding the shield, which symbolizes Oregon's admission into the Union.","The date of 1859 is found below the shield,","and this was the year that Oregon achieved statehood."])
oregon = Object("Oregon",oregonData)
#pennsylvania
pennsylvaniaData = State(["Pennsylvania"],["Capitol: Harrisburg","Population: 49,528"],["Established in 1812","Rank in state: 9th"],["Ruffed Grouse"],["Bonasa umbellus"],["Penngift Crownvetch","Coronilla"],["Medicinal uses: Coronilla varia has been used","as a diuretic and cardio-tonic"],["Adopted in 1907","The Pennsylvania coat of arms features a shield crested by a North American bald eagle,","flanked by horses, and adorned with symbols of Pennsylvania's strengths","a ship carrying state commerce to all parts of the world; a clay-red plough,","a symbol of Pennsylvania's rich natural resources; and three golden sheaves."])
pennsylvania = Object("Pennsylvania",pennsylvaniaData)
#rhode island
rhode_islandData = State(["Rhode Island"],["Capitol: Providence","Population: 190,934"],["Established in 1900","Rank in state: 1st"],["Rhode Island Red"],["Gallus gallus"],["Violet","Genus Viola"],["Medicinal uses: Violet is moist and cooling","and the leaves ease inflammation,","and when used externally, soothe skin irritations and swelling."],["Adopted in 1877","Its white background corresponded to the facings on state militia uniforms worn during the Revolution.","The flag's anchor and motto were represented in Rococo style","and encircled by blue stars corresponding to the number of states in the Union."])
rhode_island = Object("Rhode Island",rhode_islandData)
#south carolina
south_carolinaData = State(["South Carolina"],["Capitol: Columba","Population: 136,632"],["Established in 1790","Rank in state: 2nd"],["Carolina wren"],["Thryothorus ludovicianus"],["Yellow Jessamine","Gelsemium"],["Medicinal uses: Yellow jasmine has a strong tranquilizing action,","so it became a popular pain reliever and sedative,","as well as an antispasmodic for asthma,","whooping cough, and croup."],["The flag of South Carolina is a symbol of the U.S. state of South Carolina consisting of a blue field with a white palmetto tree and white crescent. ","The crescent symbol represents the silver emblem worn on the caps of South Carolina troops during the revolutionary war.","The Sabal Palmetto is a symbol of courage and strength.","During the Revolutionary War's Battle of Sullivan's Island,","a fort was crafted out of horizontal palmetto trunks. Adopted in 1861."])
south_carolina = Object("South Carolina",south_carolinaData)
#south dakota
south_dakotaData = State(["South Dakota"],["Capitol: Pierre","Population: 13,646"],["Established in 1889","Rank in state:8th"],["Ring-necked pheasant"],["Phasianus colchicus"],["American Pasque","Anemone"],["Medicinal uses: Pasque also increases venous circulation,","to induce sweating and break fever (diaphoretic),","and eruptive infections."],["Flag was adopted in 1909","The symbols on the great seal of South Dakota represent the state's commerce,","industry, and natural resources. Under God the People Rule South Dakota's","state motto appears at the top of the inner circle, which has a background of sky and hills."])
south_dakota = Object("South Dakota",south_dakotaData)
#tennessee
tennesseeData = State(["Tennessee"],["Capitol: Nashville","Population: 689,447"],["Established in 1826","Rank in state: 1st"],["Northern mockingbird"],["Mimus polyglottos"],["Tennessee Coneflower","Coneflower"],["Medicinal uses: Echinacea was used during","the 18th and 19th centuries to treat a variety of ailments","including malaria, blood poisoning,","and syphilis."],["Adopted in 1905.","The designer chose white for purity, blue to denote respect for Tennessee,","red as the traditional color for America; stars to symbolize the state's three Grand Divisions;","wheat for agricultural heritage; and the gavel for the power of the people vested in the state's legislative body."])
tennessee = Object("Tennessee",tennesseeData)
#texas
texasData = State(["Texas"],["Capitol: Austin","Population: 961,855"],["Established in 1845","Rank in state: 4th"],["Northern mockingbird"],["Mimus polyglottos"],["Bluebonnet","Lupine"],["Medicinal uses: Bluebonnets can be used as a legume,","bluebonnets have roots that work with a bacterium","called Rhizobium to improve plant growth and flowering."],["Flag was adopted in 1836","It specified that the flag should consist of a blue perpendicular stripe of the width","of one-third of the whole length of the flag and a white star of five points in the center","thereof and two horizontal stripes of equal length and breadth, the upper stripe of white,","the lower of red, of the length of two thirds of the length of the whole flag."])
texas = Object("Texas",texasData)
#utah
utahData = State(["Utah"],["Capitol: Salt Lake City","Population: 199,723"],["Established in 1856","Rank in state: 1st"],["California Gull"],["Larus californicus"],["Sego Lily","Calochortus"],["Medicinal uses: When eaten, the palatability of the herbage","is good for sheep and fair for cattle.","Horses do not graze it. The bulbs are eaten,","and also gathered and stored by pocket gophers and other rodents."],["Adopted in 2011.","According to the organization, the gold beehive represents Utah as the Beehive State and the state's motto of Industry.","The star below the beehive represents Utah's statehood and joining of the Union in 1896,","and the triangular saltire symbolizes Utah's moniker as the Crossroads of the West."])
utah = Object("Utah",utahData) 
#vermont
vermontData = State(["Vermont"],["Capitol: Montpelier","Population: 226,610"],["Established in 1780","Rank in state: 1st"],["Hermit thrush"],["Catharus guttatus"],["Red Clover","Clover"],["Medicinal uses: It has been used to treat whooping cough",",respiratory problems,","and skin inflammations."],["Adopted in 1923","Multiple versions of the flag have been included throughout history.","Originally, the flag was the same as the flag of the Green Mountain Boys."])
vermont = Object("Vermont",vermontData)
#virginia
virginiaData = State(["Virginia"],["Capitol: Richmond","Population: 226,610"],["Established in 1780","Rank in state: 4th"],["Northern Cardinal"],["Cardinalis cardinalis"],["American Dogwood","Cornus"],["Medicinal uses: People use dogwood for headaches,","fatigue, fever, and ongoing diarrhea.","It is also used to increase strength,","to stimulate appetite, and as a tonic.","Some people apply American dogwood directly","to the skin for boils and wounds"],["Adopted in 1861.","The reverse showed women symbolizing liberty, eternity, and agriculture.","The design on the obverse now appears on the state flag.","It features a woman personifying virtue and dressed as an Amazon.","She wears a helmet and holds a spear and sword above the Latin motto “Sic semper tyrannis”","(“Thus always to tyrants”)."])
colorado = Object("Virginia",virginiaData)
#washington
washingtonData = State(["Washington"],["Capitol: Olympia","Population: 46,478"],["Established in 1885","Rank in state: 24th"],["Willow goldfinch"],["Spinus tristis"],["Coast Rhododendron","Rhododendron L."],["Medicinal uses: Rhododendron is one of","the naturally occurring plants which possess","various health benefits, such as prevention and","treatment of diseases associated with heart,","dysentery, diarrhea, detoxification,","inflammation, fever, constipation,","bronchitis and asthma."],["Adopted 1923.","The flag of Washington consists of the state seal, displaying an image of its namesake George Washington,","on a field of dark green with gold fringe being optional.","It is the only U.S. state flag with a field of green as well as","the only state flag with the image of an American president."])
washington = Object("Washington",washingtonData)
#west virginia
west_virginiaData = State(["West Virginia"],["Capitol: Charleston","Population: 51,400"],["Established in 1885","Rank in state: 1st"],["Northern cardinal"],["Cardinalis cardinalisa"],["Rhododendron","Rhododendron"],["Medicinal uses: Rhododendron is one of the naturally","occurring plants which possess various health benefits,","such as prevention and treatment of diseases associated with heart,","dysentery, diarrhea, detoxification, inflammation,","fever, constipation, bronchitis and asthma."],["Adopted in 1929.","The current state flag of West Virginia consists of a pure white field bordered on four sides by a stripe of blue.","The white of the field symbolizes purity, while the blue border represents the Union.","The present flag consists of a pure white field bordered by a blue stripe with the","coat of arms of West Virginia in the center, wreathed by Rhododendron maximum and topped","by an unfurled red ribbon reading, State of West Virginia. It is the only state flag to bear crossing rifles."])
west_virginia= Object("West Virginia",west_virginiaData)
#wisconsin
wisconsinData = State(["Wisconsin"],["Capitol: Madison","Population: 269,840"],["Established in 1838","Rank in state: 2nd"],["American robin"],["Turdus migratoriusn"],["Wood Violet","Violetn"],["Medicinal uses: Violet is moist and cooling","and the leaves ease inflammation,","and when used externally,","soothe skin irritations and swelling.","It has an affinity for the lymphatic system and can","promote healthy lymphatic function. "],["Adopted in 1848.","The tools of the important trades of the times lie within the shield:","the symbols for agriculture (plow), mining (pick and shovel), manufacturing (arm and hammer),","and navigation (anchor). The badger, which is the state animal, sits above the shield.","It represents the name given to the lead miners."])
wisconsin = Object("Wisconsin",wisconsinData)
#wyoming
wyomingData = State(["Wyoming"],["Capitol: Cheyenne","Population: 64,019"],["Established in 1869","Rank in state: 1st"],["Western meadowlark"],["Sturnella neglectag"],["Indian Paintbrush","Castillejag"],["Medicinal uses: People use the Indian Paintbrush","as a medicine to treat rheumatism and as a","bath rinse to make their hair glossy."],["Flag was adopted in 1917","The flag of the state of Wyoming consists of the silhouette of an American bison.","The red symbolizes the Native Americans and the blood of pioneers who gave their lives. The white is a symbol of purity and uprightness."])
